...
axiosInstanceadmin.interceptors.response.use((response) => response, // Forward successful responses
async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
        ...
        try {
            ...
            const response = await axiosInstanceadmin.post('/refresh', {}, { withCredentials: true });
